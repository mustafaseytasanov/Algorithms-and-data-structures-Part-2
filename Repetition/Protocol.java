...
оно потеряет свою актуальность. Протокол взаимодействия клиента (компьютера, запрашивающего точное время) и 
...
сервера клиенту одинаковы, может определить и установить себе точное время, используя известные значения A, B, C.

Вам предстоит реализовать алгоритм, с точностью до секунды определяющий точное время для установки на 
клиенте по известным A, B и C. При необходимости округлите результат до целого числа секунд по правилам 
арифметики (в меньшую сторону, если дробная часть числа меньше 1/2, иначе в большую сторону).
...
public class Protocol {
    public static void main(String[] args) throws IOException {
        ...
        int timeA = 3600 * hh + 60 * mm + ss;
        str = reader.readLine();
        arr = str.split(":");
        hh = Integer.parseInt(arr[0]);
        mm = Integer.parseInt(arr[1]);
        ss = Integer.parseInt(arr[2]);
        int timeB = 3600 * hh + 60 * mm + ss;
        str = reader.readLine();
        arr = str.split(":");
        hh = Integer.parseInt(arr[0]);
        mm = Integer.parseInt(arr[1]);
        ss = Integer.parseInt(arr[2]);
        int timeC = 3600 * hh + 60 * mm + ss;
        if (timeC < timeA) {
            timeC += 3600 * 24;
        }
        int delay = (int) Math.round(((double) (timeC - timeA)) / 2);
        int exactTime = timeB + delay;
        if (exactTime >= 3600 * 24) {
            exactTime -= 3600 * 24;
        }
        hh = exactTime / 3600;
        int residue = exactTime % 3600;
        mm = residue / 60;
        ss = residue % 60;
        if (hh < 10) {
            writer.write("0" + hh + ":");
